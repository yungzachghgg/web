...
if (DATABASE_URL) {
    try {
        ...
        console.log('[DB] PostgreSQL not available:', e.message);
        console.log('[DB] Using JSON files instead');
    }
} else {
    console.log('[DB] No DATABASE_URL found, using JSON files');
}
...
app.post('/api/auth/register', async (req, res) => {
    ...
    if (usePostgres && pool) {
        try {
            console.log(`[DB] Attempting to save user: ${email}`);
            const result = await pool.query(
                'INSERT INTO users (id, username, email, password_hash, is_admin) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (email) DO UPDATE SET password_hash = $4 RETURNING *',
                [userId, username, email.toLowerCase(), hashedPassword, false]
            );
            console.log(`[DB] User saved successfully: ${email}, rows: ${result.rowCount}`);
        } catch (dbError) {
            console.error('[DB] Failed to save user to PostgreSQL:', dbError.message);
            console.error('[DB] Error details:', dbError);
        }
    } else {
        console.log(`[DB] PostgreSQL not available, user only in memory: ${email}`);
    }
